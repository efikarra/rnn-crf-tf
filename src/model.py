...
class RNN(object):
    ...
    def compute_accuracy(self, labels):
        correct_pred = tf.equal(labels, tf.cast(self.targets, tf.int32))
    ...
    def compute_labels(self, logits):
        viterbi_sequence, viterbi_score = tf.contrib.crf.crf_decode(logits, self.transition_params,
    ...
    def build_graph(self, hparams):
        ...
        with tf.variable_scope("rnn_model", dtype=dtype):
            ...
            if self.mode == tf.contrib.learn.ModeKeys.INFER:
                ...
                log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(logits, self.targets,
                                                                                      self.input_sequence_length)
        # return logits, loss

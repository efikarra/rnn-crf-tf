...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocab_path", type=str, default=None,
                        help="Vocabulary input file path.")
    parser.add_argument("--embed_path", type=str, default=None,
                        help="Input embedding file path.")
    parser.add_argument("--out_path", type=str, default=None,
                        help="Output pat.")

    flags, unparsed = parser.parse_known_args()
    vocab, _ = vocab_utils.load_vocab(flags.vocab_path)
    unk = u"<unk>"
    pad = u"<pad>"
    if vocab[0] != pad or vocab[1] != unk:
        vocab = [pad, unk] + vocab
    save_embedding(vocab, flags.flags.embed_path, flags.out_path +
                   flags.embed_path.split("/")[-1].replace(".txt", "_") + flags.vocab_path.split("/")[-1])
